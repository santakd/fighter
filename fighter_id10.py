...
# =============================================================================
# Download: python3 fighter_id10.py --mode download --source kaggle --dataset-slug kadirkrtls/tez-set-v1
# Train model: python3 fighter_id10.py --mode train --data-dir ./aircraft_data --epochs 60 --batch-size 16 --patience 10
# Inference: python3 fighter_id10.py --mode infer --model-path fighter.pth --infer-image s_1.jpg
# =============================================================================
...
def get_dataloaders(data_dir: str, batch_size: int, logger: logging.Logger):
    ...
    # Test dataset uses the same transformations as validation to ensure consistent evaluation conditions, allowing us to reliably assess the model's performance on 
    # unseen data without the influence of random augmentations that are only applied during training for better generalization.  
...
def train_model(train_loader, val_loader, test_loader, num_classes: int, max_epochs: int, patience: int, device, logger, model_path: str):
    ...
    # Compute class weights for the training dataset to handle class imbalance, which is common in real-world datasets where some classes may have significantly more samples than others.
    # By assigning higher weights to underrepresented classes, we can help the model learn to recognize them better and improve overall performance, especially on minority classes. This is crucial for achieving good accuracy across all classes, rather than just the majority ones.
    ...
    # The cosine annealing schedule allows for a gradual reduction in learning rate, which can help the model fine-tune its weights as it approaches convergence.   
    ...
    for epoch in range(max_epochs):
        ...
        # Calculate training accuracy for the epoch and log it. This provides insight into how well the model is learning from the training data, 
        # allowing us to monitor progress and identify potential issues such as overfitting or underfitting early in the training process.
        train_acc = train_correct / len(train_loader.dataset)

        # Evaluate on the validation set to calculate validation accuracy. This is crucial for monitoring the model's performance on unseen data during training,
        # allowing us to implement early stopping and save the best model based on validation performance, which is a more reliable indicator of generalization than training accuracy alone.
        model.eval()
        ...
        # This provides a comprehensive overview of the training progress and allows us to monitor how the model is improving over time, as well as how the learning rate is evolving with the scheduler.
...
def main():
    ...
    # This allows users to easily manage the entire workflow from data acquisition to model training and inference using a single script with clear options.
