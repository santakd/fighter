...
# =============================================================================
# Fighter Aircraft Identifier - LEVEL 1 UPGRADED
# Strong Augmentation + Weighted Loss + Cosine Scheduler + Final Test Eval
# Real fighter names (F-35, Rafale, F-22, etc.)
# 1. Clean old files
# rm -rf aircraft_data best_model.pth fighter_id4_*.log
#
# 2. Download clean dataset
# python3 fighter_id4.py --mode download --source kaggle
#
# 3. Train (this is the upgraded pipeline)
# python3 fighter_id4.py --mode train --epochs 30 --batch-size 16
# =============================================================================
...
def find_image_root(data_dir: str, logger: logging.Logger):
    """Auto-detect the crop folder from the clean dataset."""
...
def download_kaggle_dataset(dataset_slug: str, data_dir: str, logger: logging.Logger):
    """Download the clean fighter dataset."""
    try:
        try:
...
def train_model(train_loader, val_loader, test_loader, num_classes: int, max_epochs: int, device, logger, model_path: str):
    ...
    logger.info(f"Starting upgraded training (max {max_epochs} epochs)")
...
def main():
    parser = argparse.ArgumentParser(description="Fighter Aircraft Identifier - Level 1 Upgraded")
    ...
    try:
        ...
        if args.mode == "download":
            ...
            train_model(train_loader, val_loader, test_loader, len(class_names), args.epochs, device, logger, args.model_path)
